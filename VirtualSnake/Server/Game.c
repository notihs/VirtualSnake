...
void initGame() {
	...
	if (game == NULL) {
		...
		_tprintf(TEXT("Erro na alocacao de memoria de Game* !"));
		exit(0);
	}
...
void initSnakes() {
	...
	if (game->snake == NULL) {
		...
		_tprintf(TEXT("Erro na alocacao de memoria de Snake** !"));
		exit(0);
	}
	...
	for (int i = 0; i < MAX_PLAYERS; i++) {
		...
		if (game->snake[i]->bodyRow == NULL) {
			...
			_tprintf(TEXT("Erro na alocacao de memoria de int* (bodyRow) !"));
			exit(0);
		}
		...
		if (game->snake[i]->bodyColumn == NULL) {
			...
			_tprintf(TEXT("Erro na alocacao de memoria de int* (bodyColumn) !"));
			exit(0);
		}
...
void showSnakeInfo() {
	...
	for (int i = 0; i < game->activePlayers; i++) {
		...
		_tprintf(TEXT("ID -> %d\n"), game->snake[i]->id);
		_tprintf(TEXT("ALIVE -> %d\n"), game->snake[i]->alive);
		_tprintf(TEXT("SIZE -> %d\n"), game->snake[i]->size);
		_tprintf(TEXT("SPEED -> %lf\n"), game->snake[i]->speed);
		_tprintf(TEXT("BODY POSITIONS: \n"));

		for (int j = 0; j < game->snake[j]->size; j++) {
			_tprintf(TEXT("ROW %d -> %d\n"), j, game->snake[i]->bodyRow[j]);
			_tprintf(TEXT("COLUMN %d -> %d\n"), j, game->snake[i]->bodyColumn[j]);
...
boolean validMovement(int player, TCHAR keyPressed) {
...
void tryToMoveSnake(int player, TCHAR keyPressed) {
	...
	//showSnakeInfo();
	showMap(game->map);
	if (game->snake[player]->alive == FALSE) {
		ptrKeysInMemory[player] = '-';
	}
...
void eatingOil(int player, int rowMovement, int columnMovement) {
	...
	generateItems(game->map, MAX_NUMBER_OF_OBJECTS - 1);
	game->snake[player]->effectTimer = DEFAULT_EFFECT_TIMER;
...
void opponentGlued(int player, int rowMovement, int columnMovement) {
	...
	for (int i = 0; i < game->activePlayers; i++) {
		...
		if (i != player) {
			...
			game->snake[i]->effectTimer = DEFAULT_EFFECT_TIMER;
		}
	}
